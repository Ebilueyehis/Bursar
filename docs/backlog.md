...
| --- | --- | --- | --- |
| Backup setup: keys, B2 bucket, secrets, first run | S | building | Operator task, not code. Steps in `docs/runbooks/backup-setup.md`. `SUPABASE_DB_URL` fixed 2026-09-02 (session pooler username needed the project-ref suffix, password needed percent-encoding). The real work succeeds end to end: `pg_dump`, `age` encryption, and the B2 upload all complete, with a dated object confirmed in `daily/`. The keep-alive step's 401 was never a bad key: `curl -i` against the REST endpoint returned `42501 permission denied for function auth_school_id` — every RLS policy calls `auth_school_id()`/`auth_role()`, and EXECUTE on those is granted only to `authenticated`, never `anon`, so an anon-only request against any RLS-protected table always fails RLS evaluation itself. Correct behaviour for the security boundary (anon genuinely cannot read school data), wrong assumption in the workflow's own comment. Fixed by pointing the keep-alive ping at `/auth/v1/health` instead, which touches no table. Next: re-run and confirm green, then the restore drill. |
