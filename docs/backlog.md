...
| --- | --- | --- | --- |
| Backup setup: keys, B2 bucket, secrets, first run | S | building | Operator task, not code. Steps in `docs/runbooks/backup-setup.md`. `SUPABASE_DB_URL` fixed 2026-09-02 (session pooler username needed the project-ref suffix, password needed percent-encoding). As of run 33601304105 the real work now succeeds end to end: `pg_dump`, `age` encryption, and the B2 upload all complete, with a dated object confirmed in `daily/`. Only the last step (the anti-pause keep-alive curl against Supabase's REST API) still fails, with a 401 — `SUPABASE_ANON_KEY` is wrong, stale, or was swapped with `SUPABASE_URL` when the secrets were entered. Low stakes (doesn't block the backup itself, only the auto-pause guard) but worth fixing before the free project sits idle a week. |
